# Отчет по лабораторным работам 
**Студент:** Зиневич Роман  
**Группа:** ЭВМб-23-1  

## Лабораторная работа 1
**Вариант 4:** 

### Практическая часть
1. Что любит Бет:

![img](pictures/1.png)

2. Проверка, любит ли Мэри кукурузу:

![img](pictures/2.png)

3. Какие фрукты известны системе:

![img](pictures/3.png)

4. Какого цвета персик:

![img](pictures/4.png)

**Итог:** Освоен базовый синтаксис Пролога. Реализованы логические правила с конъюнкцией условий.

## Лабораторная работа 2
**Вариант 17:** Добавить элемент в конец списка.  
**Вариант 7:** Определить, являются ли два заданных элемента соседними в списке.

**Декларативная интерпретация (Вар 17):** Добавление элемента в пустой список дает список из этого одного элемента. Иначе - результат состоит из старой головы и хвоста, к которому добавлен элемент.  
**Процедурная интерпретация (Вар 17):** Программа рекурсивно проходит до конца списка (отделяя голову), а затем прикрепляет новый элемент к пустому хвосту.

**Декларативная интерпретация (Вар 7):** Два элемента являются соседними, если они стоят первыми в списке, либо если они являются соседними в хвосте этого списка.  
**Процедурная интерпретация (Вар 7):** Программа проверяет первые два элемента списка. Если совпали с искомыми - возвращает истину. Иначе отбрасывает первую голову и рекурсивно повторяет проверку для оставшегося хвоста.

### Практическая часть
1. Добавление элемента в заполненный список (Вар 17):
![img](pictures/5.png)

2. Добавление элемента в пустой список (Вар 17):
![img](pictures/6.png)

3. Проверка соседства элементов (успех) (Вар 7):
![img](pictures/20.png)

4. Проверка соседства элементов (неудача) (Вар 7):
![img](pictures/21.png)

**Итог:** Изучена рекурсивная обработка списков, освоена работа с конструкцией `[H|T]`.

## Лабораторная работа 3
**Вариант 11:** Кинотеатры и фильмы.

### Практическая часть
1. Первичная загрузка тестовой базы данных:

![img](pictures/7.png)

2. Вывод доступных фильмов:

![img](pictures/8.png)

3. Добавление новой записи фильма:

![img](pictures/9.png)

4. Каскадное удаление (фильм + сеансы):

![img](pictures/10.png)

5. Выход из программы:

![img](pictures/11.png)

**Итог:** Реализовано консольное интерактивное меню. Опробованы динамические предикаты и функции `assertz`, `retractall` для изменения состояния памяти.

## Лабораторная работа 4
**Вариант 5:** Решить задачу о выполнимости логической функции `x1*x2*!x3 v !x1*x2*x3`.

### Практическая часть
1. Вывод всех истинных наборов переменных:

![img](pictures/12.png)

2. Трудоемкость полного перебора:

![img](pictures/13.png)

3. Трудоемкость оптимизированного перебора:

![img](pictures/14.png)

**Итог:** Анализ формулы показал, что переменная `x2` всегда равна 1, а `x1` и `x3` имеют противоположные значения. Применение этого математического факта сократило пространство поиска и снизило количество логических выводов (inferences).

## Лабораторная работа 5
**Задание:** Создать ЭС на тему "Выбор домашнего питомца".

### Практическая часть
1. Ветвь 1:

![img](pictures/17.png)

2. Ветвь 2:

![img](pictures/15.png)

3. Ветвь 3:

![img](pictures/16.png)

4. Ветвь 4:
   
![img](pictures/18.png)

5. Срабатывание правила защиты от некорректного ввода:
   
![img](pictures/19.png)

**Итог:** Изучена среда CLIPS. Написаны продукционные правила `defrule`, 4 логические ветки и правила, перехватывающие неверные ответы (`~yes&~no`).
